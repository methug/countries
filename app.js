...
const specific = function () {
    const darkMode = () => {
        const elements = []
        elements.push(countryName, nativeName, population, region, subRegion, capital, domain, currencies, languages, indication, ...borderC)
        for (let each of elements) {
            each.classList.add('light')
        }
    }
    const countryName = document.createElement('h2')
    countryName.innerText = this.children[1].children[0].children[0].innerText
    const specificCountry = data.filter(x => x.name === countryName.innerText)
    mainContainer.remove()
    const main = document.createElement('main')
    main.classList.add('cont')
    document.body.append(main)
    const flag = document.createElement('img')
    flag.classList.add('flag-expanded')
    let flagStyle = this.children[0].getAttribute('style')
    let corrected = flagStyle.slice(flagStyle.indexOf(`h`), flagStyle.lastIndexOf('"'))
    flag.src = corrected
    const explanation = document.createElement('div')
    explanation.classList.add('explanation')
    const details = document.createElement('div')
    details.classList.add('detail-explanation')
    const nativeName = document.createElement('span')
    nativeName.innerText = specificCountry[0].nativeName
    const population = document.createElement('span')
    population.innerText = this.children[1].children[0].children[1].innerText
    const region = document.createElement('span')
    region.innerText = this.children[1].children[0].children[2].innerText
    const subRegion = document.createElement('span')
    subRegion.innerText = specificCountry[0].subregion
    const capital = document.createElement('span')
    capital.innerText = this.children[1].children[0].children[3].innerText
    const domain = document.createElement('span')
    domain.innerText = specificCountry[0].demonym
    const currencies = document.createElement('span')
    currencies.innerText = specificCountry[0].currencies
    const languages = document.createElement('span')
    languages.innerText = specificCountry[0].languages[0].name
    const borderCountries = document.createElement('footer')
    borderCountries.classList.add('border-countries')
    const indication = document.createElement('span')
    indication.innerText = 'Border Countries:'
    const borderC1 = document.createElement('span')
    const borderC2 = document.createElement('span')
    const borderC3 = document.createElement('span')
    const borderC = [];
    borderC.push(borderC1, borderC2, borderC3)
    if (specificCountry[0].borders) {
        for (let i = 0; i < 3; i++) {
            borderC[i].innerText = specificCountry[0].borders[i]
        }
    }

    details.append(...[nativeName, population, region, subRegion, capital, domain, currencies, languages])
    borderCountries.append(...[indication, borderC1, borderC2, borderC3])
    explanation.append(...[countryName, details, borderCountries])
    main.append(...[flag, explanation])
    modeSwitcher.addEventListener('click', darkMode)
}



function createPage(dataStructure = data) {
    for (let i = 0; i < dataStructure.length; i++) {
        const countryContainer = document.createElement('div')
        countryContainer.classList.add('each-country-container')
        countryContainer.addEventListener('click', specific)
        ...
        populationTitle.classList.add('info-text')
        regionTitle.innerText = 'Region:'
        regionTitle.classList.add('info-text')
        capitalTitle.innerText = 'Capital:'
        capitalTitle.classList.add('info-text')
        countryName.innerText = dataStructure[i].name
        countryName.classList.add('country-name')
        population.append(...[populationTitle, dataStructure[i].population])
        region.append(...[regionTitle, dataStructure[i].region])
        if (dataStructure[i].capital) {
            capital.append(...[capitalTitle, dataStructure[i].capital])
        }
        innerDescription.append(...[countryName, population, region, capital])
        description.append(innerDescription)
        flag.style.backgroundImage = `url(${dataStructure[i].flags.png})`
        countryContainer.append(...[flag, description])
        itemsContainer.append(countryContainer)
    }
    return itemsContainer
}

const itemsContainer = document.querySelector('.countries')
const modeSwitcher = document.querySelector('.mode-switcher')
const navBar = document.querySelector('.top-nav')
const heading = document.querySelector('.main-heading')
const inputContainer = document.querySelector('.inp-container')
const input = document.querySelector('#country')
const label = document.querySelector('label[for="region-select"]')
const selection = document.querySelector('.region-select')

modeSwitcher.addEventListener('click', function () {
    document.body.classList.toggle('dark-background')
    navBar.classList.toggle('dark-nav')
    heading.classList.toggle('for-dark-text')
    inputContainer.classList.toggle('dark-input')
    input.classList.toggle('dark-input')
    label.classList.toggle('for-dark-text')
    selection.classList.toggle('dark-input')
    this.classList.toggle('for-dark-text')
    for (let each of itemsContainer.children) {
        each.classList.toggle('dark-input')
...
const display = (value) => {
    if (value !== 'all') {
        clearPage()
        let givenData = data.filter(x => x.region === value)
        return createPage(givenData)
    }
    else {
        clearPage()
        createPage()
...
input.addEventListener('input', function () {
    ...
    let givenData = data.filter(x => x.name.includes(value))
    if (value && givenData.length !== 0) {
        clearPage()
        createPage(givenData)
    } else if (value && givenData.length === 0) {
        clearPage()
        const warning = document.createElement('p')
        warning.innerText = 'Sorry, This Country Does not exist'
        warning.classList.add('warning')
        itemsContainer.append(warning)
    }
    if (value === '') {
        clearPage()
        createPage()
